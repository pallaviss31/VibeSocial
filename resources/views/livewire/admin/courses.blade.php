    ...
    @if ($showForm)
        <div class="bg-white border rounded-xl p-6 mb-8">
            <form wire:submit.prevent="createCourse" class="space-y-4">
                ...
                <div>
                    <label class="text-xs font-bold">Course Name</label>
                ...
                <div>
                    <label class="text-xs font-bold">Semester</label>
                    <input type="text" wire:model="semester" class="w-full border rounded px-3 py-2">
                </div>

                <div>
                    <label class="text-xs font-bold">Year</label>
                    <input type="text" wire:model="year" class="w-full border rounded px-3 py-2">
                </div>
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($courses as $course)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4">
                            ...
                            <div class="text-[11px] text-slate-400 italic">Added
                                {{ $course->created_at?->format('M d, Y') ?? 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <span
                        ...
                        <td class="px-6 py-4 text-right">
                            <button wire:click="deleteCourse({{ $course->id }})" wire:confirm="Delete this course?"
                ...
                @empty
                    <tr>
                        <td colspan="3"
                            class="px-6 py-10 text-center text-slate-400 text-xs uppercase font-bold tracking-widest">No
                            courses found</td>
                    </tr>
                @endforelse
            </tbody>
